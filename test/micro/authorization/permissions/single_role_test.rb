...
class Micro::Authorization::Permissions::SingleRoleTest < Minitest::Test
  ...
  class TestAdminPermissions < BaseTest
    ...
    def test_role_permissions
      permissions1 = build_permissions(context: 'home')
      permissions2 = build_permissions(context: ['home'])
  ...
  class TestReadonlyPermissions < BaseTest
  ...
  class TestUser0Permissions < BaseTest
    ROLE = <<-JSON
      {
        "name": "user0",
        "permissions": {
    ...
    def test_role_permissions
      permissions1 = build_permissions(context: 'invoices')
      ...
      assert permissions_to.context?('invoices')

      permissions2 = build_permissions(context: ['sales'])
      ...
      refute another_permissions_to.context?('sales')
    end
  end

  class TestUser1Permissions < BaseTest
    ROLE = <<-JSON
      {
        "name": "user1",
        "permissions": {
    ...
    def test_role_permissions
      ...
      permissions3 = build_permissions(context: 'commissionings')

      assert permissions3.to?('visit')
      refute permissions3.to?('export')
      refute permissions3.to?(['visit', 'export'])

      permissions4 = build_permissions(context: 'commissionings')
  ...
  class TestUser2Permissions < BaseTest
    ROLE = <<-JSON
      {
        "name": "user2",
        "permissions": {
  ...
  class TestUser3Permissions < BaseTest
    ROLE =  <<-JSON
      {
        "name": "user3",
        "permissions": {
  ...
  class TestAuthorizationPermissionsToHanamiClasses < BaseTest
    module Dashboard
      module Controllers
        module Sales
          class Index
    ...
    def test_role_permissions
      ...
      permissions1 = build_permissions(context: controller_context)
      ...
      permissions2 = build_permissions(context: view_context)
